...
export default function ProdutosPage() {
  ...
  async function refresh() {
    ...
    setLoading(true);
    try {
      const [prd, cat, cul, cc] = await Promise.all([
  ...
  async function onSave() {
    ...
    try {
      ...
      if (!editingId) {
        const created = await createProdutoEstoque(token, payload);
        setItems((prev) => [...prev, created]);
        setModalOpen(false);
        return;
      }
      const updated = await updateProdutoEstoque(token, editingId, payload);
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-300">Cadastre produtos com categoria, cultura e centro de custo.</p>
            </div>
          ...
          <section className="h-fit rounded-3xl border border-white/10 bg-white/5 p-4 shadow-[0_0_0_1px_rgba(255,255,255,0.06)_inset] backdrop-blur-xl">
            ...
            <div className="mt-3 hidden grid-cols-12 gap-3 rounded-2xl border border-white/10 bg-zinc-950/30 px-3 py-2 text-[11px] font-black uppercase tracking-[0.22em] text-zinc-400 lg:grid">
              ...
              <div className="col-span-2">Centro</div>
            </div>
            ...
            <div className={`mt-4 pr-1 ${filtered.length > 10 ? "max-h-[560px] overflow-auto" : ""}`}>
              <div className="space-y-2">
                ...
                {!loading && filtered.length === 0 ? (
                  <div className="rounded-2xl border border-white/10 bg-zinc-950/30 p-4 text-sm text-zinc-300">Nenhum produto encontrado.</div>
                ) : null}
              </div>
            </div>
          </section>
