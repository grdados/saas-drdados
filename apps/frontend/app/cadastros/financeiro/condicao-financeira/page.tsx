...
import {
  CondicaoFinanceira,
  createCondicaoFinanceira,
  isApiError,
  listCondicoesFinanceiras,
  updateCondicaoFinanceira
} from "@/lib/api";
...
export default function CondicaoFinanceiraPage() {
  const [loading, setLoading] = useState(true);
  const [items, setItems] = useState<CondicaoFinanceira[]>([]);
  ...
  const [formDias, setFormDias] = useState<string>("0");
  const [formParcelas, setFormParcelas] = useState<string>("1");
  ...
  const filtered = useMemo(() => {
    ...
    return [...items]
      .filter((i) => {
        ...
        if (!q) return true;
        return i.name.toLowerCase().includes(q) || String(i.dias).includes(q) || String(i.parcelas).includes(q);
      })
  ...
  async function refresh() {
    ...
    setLoading(true);
    try {
      const data = await listCondicoesFinanceiras(token);
      setItems(data);
    } catch (err) {
  ...
  function openEdit(id: number) {
    ...
    setEditingId(id);
    setFormName(it.name);
    setFormDias(String(it.dias ?? 0));
    setFormParcelas(String(it.parcelas ?? 1));
  ...
  async function onSave() {
    ...
    try {
      const payload = {
        name: formName.trim(),
        dias: Number(formDias || "0"),
        parcelas: Number(formParcelas || "1"),
        is_active: formActive
      };
      if (!editingId) {
        const created = await createCondicaoFinanceira(token, payload);
        setItems((prev) => [...prev, created]);
        setModalOpen(false);
        return;
      }
      const updated = await updateCondicaoFinanceira(token, editingId, payload);
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-black tracking-tight text-white">Condicoes financeiras</h1>
              <p className="mt-1 text-sm text-zinc-300">Cadastre condicao, dias e parcelas.</p>
            </div>
          ...
          <section className="rounded-3xl border border-white/10 bg-white/5 p-4 shadow-[0_0_0_1px_rgba(255,255,255,0.06)_inset] backdrop-blur-xl">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-zinc-400">Refine por condicao, dias e parcelas.</p>
              </div>
          ...
          <section className="h-fit rounded-3xl border border-white/10 bg-white/5 p-4 shadow-[0_0_0_1px_rgba(255,255,255,0.06)_inset] backdrop-blur-xl">
            ...
            <div className="mt-3 hidden grid-cols-12 gap-3 rounded-2xl border border-white/10 bg-zinc-950/30 px-3 py-2 text-[11px] font-black uppercase tracking-[0.22em] text-zinc-400 sm:grid">
              <div className="col-span-6">Condicao</div>
            ...
            <div className={`mt-4 pr-1 ${filtered.length > 10 ? "max-h-[560px] overflow-auto" : ""}`}>
              <div className="space-y-2">
                  ...
                  return (
                    ...
                    >
                      <button onClick={() => openEdit(it.id)} className="grid min-w-0 flex-1 grid-cols-12 items-center gap-3 text-left">
                        ...
                        <div className="col-span-2 text-sm font-black text-zinc-100">{it.dias}</div>
                        <div className="col-span-2 text-sm font-black text-zinc-100">{it.parcelas}</div>
                ...
                {!loading && filtered.length === 0 ? (
                  <div className="rounded-2xl border border-white/10 bg-zinc-950/30 p-4 text-sm text-zinc-300">Nenhuma condicao encontrada.</div>
                ) : null}
              </div>
            </div>
          </section>
            ...
            <div className="grid gap-4">
              <div className="grid gap-4 md:grid-cols-3">
                <div className="grid gap-2 md:col-span-2">
                  <label className="text-xs font-black uppercase tracking-[0.22em] text-zinc-400">Condicao</label>
                  <input
                    value={formName}
                ...
                <div className="grid gap-2">
                  <label className="text-xs font-black uppercase tracking-[0.22em] text-zinc-400">Dias</label>
                  <input
                    value={formDias}
                ...
                <div className="grid gap-2">
                  <label className="text-xs font-black uppercase tracking-[0.22em] text-zinc-400">Parcelas</label>
                  <input
                    value={formParcelas}
