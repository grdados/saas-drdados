...
export default function GrupoProdutoresPage() {
  const [loading, setLoading] = useState(true);
  const [items, setItems] = useState<GrupoProdutor[]>([]);
  ...
  const filtered = useMemo(() => {
    ...
    return [...items]
      .filter((i) => {
        ...
        if (!q) return true;
        return i.name.toLowerCase().includes(q) || (i.cpf_cnpj || "").toLowerCase().includes(q);
      })
  ...
  async function refresh() {
    ...
    setLoading(true);
    try {
      const data = await listGruposProdutores(token);
      setItems(data);
    } catch (err) {
  ...
  async function onSave() {
    ...
    setSaveMessage("");
    try {
      const payload = { name: formName.trim(), cpf_cnpj: formDoc.trim(), is_active: formActive };
      if (!editingId) {
        const created = await createGrupoProdutor(token, payload);
        setItems((prev) => [...prev, created]);
        setModalOpen(false);
        return;
      }
      const updated = await updateGrupoProdutor(token, editingId, payload);
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-black tracking-tight text-white">Grupo de produtores</h1>
              <p className="mt-1 text-sm text-zinc-300">Organize produtores por grupo e documento (CPF/CNPJ).</p>
            </div>
          ...
          <section className="rounded-3xl border border-white/10 bg-white/5 p-4 shadow-[0_0_0_1px_rgba(255,255,255,0.06)_inset] backdrop-blur-xl">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-zinc-400">Refine por grupo, CPF/CNPJ e status.</p>
              </div>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-[1fr_180px]">
              <input
                ...
                placeholder="Buscar grupo ou CPF/CNPJ..."
                    ...
                    >
                      <button onClick={() => openEdit(it.id)} className="grid min-w-0 flex-1 grid-cols-12 items-center gap-3 text-left">
                        ...
                        <div className="col-span-3 min-w-0">
                          <p className="truncate text-sm font-semibold text-zinc-100">{it.cpf_cnpj || "-"}</p>
                        </div>
            ...
            <div className="grid gap-4">
              ...
              <label className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-zinc-950/35 px-4 py-3">
                <div>
                  ...
                  <p className="text-xs text-zinc-400">Se desativado, nao aparece nas operacoes.</p>
                </div>
                <button
                  type="button"
