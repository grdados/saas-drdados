...
function statusMeta(status: Empreendimento["status"]) {
  if (status === "closed") return { label: "Entregue", cls: "border-emerald-400/30 bg-emerald-500/15 text-emerald-200" };
...
export default function EmpreendimentosPage() {
  ...
  const formTotals = useMemo(() => {
    ...
    const billing = unitBase * n(form.sale_price);
    return { kg, sc, billing };
  ...
  function setRowField(index: number, key: keyof FormRow, value: string) {
    setForm((prev) => ({
      ...prev,
      rows: prev.rows.map((r, i) => (i === index ? { ...r, [key]: value } : r))
    }));
  }
  ...
  function save() {
    ...
    if (!form.code.trim()) {
      setError("Informe o código do empreendimento.");
      return;
    }
    ...
    try {
      ...
      const entity: Empreendimento = {
        ...
        created_at: editingId ? rows.find((x) => x.id === editingId)?.created_at ?? now : now,
        updated_at: now,
        date: form.date,
        ...
        <div className="space-y-5">
          ...
          <section className="rounded-3xl border border-white/15 bg-zinc-900/55 p-4">
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              ...
              <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as "all" | Empreendimento["status"])} className="w-full rounded-2xl border border-white/10 bg-zinc-950/40 px-3 py-2.5 text-sm text-zinc-100"><option value="all" style={optionStyle}>Status</option><option value="pending" style={optionStyle}>Pendente</option><option value="closed" style={optionStyle}>Entregue</option><option value="canceled" style={optionStyle}>Cancelado</option></select>
            </div>
          ...
          {open ? (
            <div className="fixed inset-0 z-50 grid place-items-center px-4">
              ...
              <div className="relative w-full max-w-[1320px] max-h-[92vh] overflow-auto rounded-3xl border border-white/15 bg-zinc-900/95 p-5 space-y-4">
                ...
                <div className="grid gap-3 lg:grid-cols-4">
                  ...
                  <div className="grid gap-2"><label className="text-xs font-black uppercase tracking-[0.22em] text-zinc-400">Empreendimento</label><input value={form.code} onChange={(e) => setField("code", e.target.value.toUpperCase())} className="rounded-2xl border border-white/10 bg-zinc-950/40 px-3 py-2.5 text-sm text-zinc-100" /></div>
                  ...
                  <div className="grid gap-2"><label className="text-xs font-black uppercase tracking-[0.22em] text-zinc-400">UN</label><select value={form.unit} onChange={(e) => setField("unit", e.target.value as UnidadeProducao)} className="rounded-2xl border border-white/10 bg-zinc-950/40 px-3 py-2.5 text-sm text-zinc-100"><option value="KG" style={optionStyle}>KG</option><option value="SC" style={optionStyle}>SC</option></select></div>
                  <div className="grid gap-2"><label className="text-xs font-black uppercase tracking-[0.22em] text-zinc-400">Preço Venda</label><input value={form.sale_price} onChange={(e) => setField("sale_price", e.target.value)} className="rounded-2xl border border-white/10 bg-zinc-950/40 px-3 py-2.5 text-right text-sm text-zinc-100" /></div>
                  <div className="grid gap-2"><label className="text-xs font-black uppercase tracking-[0.22em] text-zinc-400">Status</label><select value={form.status} onChange={(e) => setField("status", e.target.value as Empreendimento["status"])} className="rounded-2xl border border-white/10 bg-zinc-950/40 px-3 py-2.5 text-sm text-zinc-100"><option value="pending" style={optionStyle}>Pendente</option><option value="closed" style={optionStyle}>Entregue</option><option value="canceled" style={optionStyle}>Cancelado</option></select></div>
                </div>

                <div className="grid gap-2"><label className="text-xs font-black uppercase tracking-[0.22em] text-zinc-400">Observação</label><textarea value={form.notes} onChange={(e) => setField("notes", e.target.value)} rows={2} className="w-full rounded-2xl border border-white/10 bg-zinc-950/40 px-3 py-2.5 text-sm text-zinc-100" /></div>

                <div className="hidden gap-2 text-[11px] font-black uppercase tracking-[0.22em] text-zinc-400 xl:grid xl:grid-cols-[1fr_1fr_1fr_100px_110px_130px_130px_130px_130px_auto]">
                  <div>Talhão</div><div>Produto</div><div>Cultivar</div><div>UN</div><div>Área (ha)</div><div>Produtividade</div><div>Data Plantio</div><div>Data Fechamento</div><div>Produção</div><div>Remover</div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-xs text-zinc-400">Produção estimada: {formatSc(formTotals.sc)} | {formatKg(formTotals.kg)}</p>
                    <p className="text-sm font-black text-white">Faturamento R$: {formatCurrencyBRL(formTotals.billing)}</p>
                  </div>
                </div>
