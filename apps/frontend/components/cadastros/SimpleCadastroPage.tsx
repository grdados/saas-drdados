...
type CrudApi<T extends BaseItem> = {
  list: (token: string) => Promise<T[]>;
  create: (token: string, payload: { name: string; is_active?: boolean }) => Promise<T>;
  update: (token: string, id: number, payload: { name?: string; is_active?: boolean }) => Promise<T>;
};
...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center px-4">
      ...
      <div className="relative w-full max-w-[720px] overflow-hidden rounded-3xl border border-white/15 bg-zinc-900/85 shadow-2xl">
...
export function SimpleCadastroPage<T extends BaseItem>({
  ...
  async function onSave() {
    ...
    try {
      if (!editingId) {
        await api.create(token, { name: formName.trim(), is_active: formActive });
        await refresh();
      ...
      await api.update(token, editingId, { name: formName.trim(), is_active: formActive });
      await refresh();
      setConfirmOpen(false);
      setModalOpen(false);
    } catch (err) {
  ...
  const isOverlayOpen = modalOpen || confirmOpen;

  return (
    <AuthedAdminShell hideHeader={hideHeader}>
      {() => (
        <div className="space-y-6">
          <div className={`space-y-6 transition-all duration-200 ${isOverlayOpen ? "blur-[2px] saturate-75" : ""}`}>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-[11px] font-black uppercase tracking-[0.24em] text-zinc-400">{breadcrumb}</p>
          ...
          <section className="rounded-3xl border border-white/10 bg-white/5 p-4 shadow-[0_0_0_1px_rgba(255,255,255,0.06)_inset] backdrop-blur-xl">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-zinc-400">Refine por nome e status.</p>
              </div>
            ...
            {tableMode ? (
              <div className="mt-3 overflow-x-auto">
                <div className="min-w-[860px] space-y-2">
                  <div className="grid grid-cols-[2fr_1fr_1fr_64px] items-center gap-3 rounded-2xl border border-white/10 bg-zinc-950/30 px-3 py-2 text-[10px] font-black uppercase tracking-[0.22em] text-zinc-400">
                    <div>{fieldLabel}</div>
                    <div>Status</div>
                    <div>Atualizado</div>
                  ...
                  {filtered.map((it) => {
                    ...
                    return (
                      <div
                        key={it.id}
                        className="grid grid-cols-[2fr_1fr_1fr_64px] items-center gap-3 rounded-2xl border border-white/10 bg-zinc-950/35 px-3 py-2.5 text-[10px]"
                      >
                        ...
                        <div className="whitespace-nowrap text-zinc-300">{new Date(it.updated_at).toLocaleDateString("pt-BR")}</div>
                  ...
                  {!loading && filtered.length === 0 ? (
                    <div className="rounded-2xl border border-white/10 bg-zinc-950/30 p-3 text-xs text-zinc-300">Nenhum item encontrado.</div>
                  ) : null}
                </div>
              </div>
            ) : (
              <div className={`mt-4 pr-1 ${filtered.length > 10 ? "max-h-[560px] overflow-auto" : ""}`}>
                <div className="mt-3 hidden grid-cols-12 gap-3 rounded-2xl border border-white/10 bg-zinc-950/30 px-3 py-2 text-[11px] font-black uppercase tracking-[0.22em] text-zinc-400 sm:grid">
                  <div className="col-span-8">{fieldLabel}</div>
                ...
                <div className="mt-2 space-y-2">
                  ...
                  {!loading && filtered.length === 0 ? (
                    <div className="rounded-2xl border border-white/10 bg-zinc-950/30 p-4 text-sm text-zinc-300">Nenhum item encontrado.</div>
                  ) : null}
                </div>
              </div>
            )}
                ...
                <div className="border-b border-white/10 p-5">
                  ...
                  <div className="mt-3 rounded-2xl border border-emerald-400/30 bg-emerald-500/10 p-3">
                    <p className="text-sm font-semibold text-emerald-100">
                      {editingId
                        ? `Deseja salvar as alterações de ${title.toLowerCase()}?`
                        : `Deseja confirmar novo ${title.toLowerCase()}?`}
                    </p>
                  </div>
                </div>
